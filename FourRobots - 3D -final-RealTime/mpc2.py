...
def predictbm(node,_3dmap):#该代码表示向最下层中间节点
    try:
        if ((_3dmap[node.x][node.y - 1][node.z] == '0') and (_3dmap[node.x][node.y - 2][node.z] == '0' or _3dmap[node.x][node.y - 2][node.z] == '1' or _3dmap[node.x][node.y - 2][node.z] == '5' or _3dmap[node.x][node.y - 2][node.z] == '7' or _3dmap[node.x][node.y - 2][node.z] == '4')):
            flag =0                         #下一步为空闲状态，并且下下一步为空闲状态或R1的目标点
            flag2 = 0
            return False, flag,flag2
        elif (_3dmap[node.x][node.y - 1][node.z] == '4'):   #下一步为目标点
            flag = 0
            flag2 = 0
...
def predictbr(node,_3dmap):#该代码表示向最下层右节点
    try:
        if ((_3dmap[node.x + 1][node.y - 1][node.z] == '0') and (_3dmap[node.x + 2][node.y - 2][node.z] == '0' or _3dmap[node.x + 2][node.y - 2][node.z] == '1' or _3dmap[node.x + 2][node.y - 2][node.z] == '5' or _3dmap[node.x + 2][node.y - 2][node.z] == '7' or _3dmap[node.x + 2][node.y - 2][node.z] == '4')):
            flag2 = 0
            ...
            flag = 0
            return False,flag,flag2
        elif (_3dmap[node.x + 1][node.y - 1][node.z] == '4'):   #下一步为目标点
            flag2 = 0
...
def predictbu(node,_3dmap):#该代码表示最下层向上
    try:
        if ((_3dmap[node.x][node.y - 1][node.z - 1] == '0') and (_3dmap[node.x][node.y - 2][node.z - 2] == '0' or _3dmap[node.x][node.y - 2][node.z - 2] == '1' or _3dmap[node.x][node.y - 2][node.z - 2] == '5' or _3dmap[node.x][node.y - 2][node.z - 2] == '7' or _3dmap[node.x][node.y - 2][node.z - 2] == '4')):
            flag2 = 0
            ...
            flag = 0
            return False,flag,flag2
        elif (_3dmap[node.x][node.y - 1][node.z - 1] == '4'):   #下一步为目标点
            flag2 = 0
...
def updatebm(node,_3dmap): #该代码表示最下层中间节点
    ...
    node.y = node.y - 1
    node.z = node.z
    _3dmap[node.x][node.y][node.z] = '3' # 更新起始状态坐标
...
def predictbrd(node,_3dmap): #该代码表示最下层向右下，则向右上，向左下，向左上同理
    try:
        if (_3dmap[node.x + 1][node.y - 1][node.z + 1] == '0' and (_3dmap[node.x + 2][node.y - 2][node.z + 2] == '0' or _3dmap[node.x + 2][node.y - 2][node.z + 2] == '1' or _3dmap[node.x + 2][node.y - 2][node.z + 2] == '5' or _3dmap[node.x + 2][node.y - 2][node.z + 2] == '7' or _3dmap[node.x + 2][node.y - 2][node.z + 2] == '4')):
            flag2 = 0
            ...
            flag = 0
            return False,flag,flag2
        elif (_3dmap[node.x + 1][node.y - 1][node.z + 1] == '4'):
            flag2 = 0
...
def predictmr(node,_3dmap):#该代码表示向中间层右节点
    try:
        if ((_3dmap[node.x + 1][node.y][node.z] == '0') and (_3dmap[node.x + 2][node.y][node.z] == '0' or _3dmap[node.x + 2][node.y][node.z] == '1' or _3dmap[node.x + 2][node.y][node.z] == '5' or _3dmap[node.x + 2][node.y][node.z] == '7' or _3dmap[node.x + 2][node.y][node.z] == '4')):
            flag =0                         #下一步为空闲状态，并且下下一步为空闲状态或其他机器人的当前位置或Rk的目标点
            flag2 = 0
            return False, flag,flag2
        elif (_3dmap[node.x + 1][node.y][node.z] == '4'):   #下一步为目标点
            flag = 0
            flag2 = 0
            return True, flag,flag2
...
def predictmu(node,_3dmap):#该代码表示中间层向上
    try:
        if ((_3dmap[node.x][node.y][node.z - 1] == '0') and (_3dmap[node.x][node.y][node.z - 2] == '0' or _3dmap[node.x][node.y][node.z - 2] == '1' or _3dmap[node.x][node.y][node.z - 2] == '5' or _3dmap[node.x][node.y][node.z - 2] == '7' or _3dmap[node.x][node.y][node.z - 2] == '4')):
            flag =0                         #下一步为空闲状态，并且下下一步为空闲状态或R1的目标点
            flag2 = 0
            return False, flag,flag2
        elif (_3dmap[node.x][node.y][node.z - 1] == '4'):   #下一步为目标点
            flag = 0
            flag2 = 0
            return True, flag,flag2
...
def predictmrd(node,_3dmap): #该代码表示中间层向右下，则向右上，向左下，向左上同理
    try:
        if (_3dmap[node.x + 1][node.y][node.z + 1] == '0' and (_3dmap[node.x + 2][node.y ][node.z + 2] == '0' or _3dmap[node.x + 2][node.y ][node.z + 2] == '1' or _3dmap[node.x + 2][node.y ][node.z + 2] == '5' or _3dmap[node.x + 2][node.y ][node.z + 2] == '7' or _3dmap[node.x + 2][node.y ][node.z + 2] == '4')):
            flag2 = 0
            ...
            flag = 0
            return False,flag,flag2
        elif (_3dmap[node.x + 1][node.y][node.z + 1] == '4'):
            flag2 = 0
...
def preset_map():
    ...
    _3dmap3.append('0 0 3 0 5 0 0'.split())
    _3dmap3.append('0 0 0 0 0 0 0'.split())
    _3dmap3.append('0 0 9 9 9 0 0'.split())
    _3dmap3.append('0 0 0 0 0 0 0'.split())
    _3dmap3.append('0 0 4 0 6 0 0'.split())
    _3dmap3.append('0 0 0 0 0 0 0'.split())
    _3dmap4 = []
    ...
    _3dmap5.append('0 0 1 0 7 0 0'.split())
    _3dmap5.append('0 0 0 0 0 0 0'.split())
    _3dmap5.append('0 0 0 0 0 0 0'.split())
    _3dmap5.append('0 0 0 0 0 0 0'.split())
    _3dmap5.append('0 0 2 0 8 0 0'.split())
    _3dmap5.append('0 0 0 0 0 0 0'.split())
    _3dmap6 = []
